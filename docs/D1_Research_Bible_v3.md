...
**Status:** **CLOSED-LOOP.** Theorem **D1★** (converse) and Theorem **D1★★** (achievability, §1.4) meet at $\min\{E^{\mathrm{cen}}(\theta),\theta_{\mathrm{IB}}(\Gamma_k)\}$ for **general time-varying directed cyclic graphs**, testing against independence — zero gap. The sole remaining phase is hardware-accelerated empirical validation (§1.7–§1.8).
...
> **Replacement:** The converse is isolated as **D1★** (§1.3, proved); achievability is **closed** as **Theorem D1★★** (§1.4) via Type-Preserving Network Coding; the "exponential-in-diameter" claim is **retracted** and replaced by the exact ergodic cut-aggregation of Lemma C-D1.
...
**Relationship between $\theta_{\mathrm{SHA}}$ and $\theta_{\mathrm{IB}}$ (corrected — no universal ordering).** $\theta_{\mathrm{SHA}}$ and $\theta_{\mathrm{IB}}$ solve **different optimization problems over different domains**: $\theta_{\mathrm{IB}}(\Gamma)=\max_{I(U;X)\le\Gamma}I(U;Y)$ **maximizes** relevance mutual information, whereas $\theta_{\mathrm{SHA}}(\Gamma)=\min_{I_P(\tilde U;X)\le\Gamma}D(P_{\tilde U}\Vert Q_{\tilde U})$ **minimizes** a KL divergence between induced marginals. **For testing against independence, $\theta_{\mathrm{SHA}}=\theta_{\mathrm{IB}}$ exactly** (the reduction shown above). **For the general pair they are separate bounds, not universally ordered** — neither $\theta_{\mathrm{SHA}}\le\theta_{\mathrm{IB}}$ nor $\theta_{\mathrm{SHA}}\ge\theta_{\mathrm{IB}}$ holds in general; each is the correct converse for its own problem. Accordingly, D1★ uses $\theta_{\mathrm{IB}}(\Gamma_k)$ for the against-independence target (where it is tight and where D1★★ closes achievability) and $\theta_{\mathrm{SHA}}(\Gamma_k)$ for the general pair.
...
- **What changes when all three of {time-varying topology, no fusion center, per-edge rate constraints} hold simultaneously:** (1) *No fusion center* removes the single-detector structure that makes AC/SHA single-letter; the exponent becomes node-dependent and coupled to consensus dynamics. (2) *Time-varying topology* makes the "available rate to node $k$" a stochastic-process functional ($\Gamma_k$), not a fixed number; the cut that binds changes with $t$. (3) *Per-edge rate constraints* couple (1) and (2): the consensus that fixes (1) must itself be carried over the very links constrained by (2), so the compression and the agreement are not separable. The **intersection** had no published characterization when this program began; it is **now characterized in full** — D1★ (converse, §1.3) and **Theorem D1★★** (achievability, §1.4) together give the exact exponent $\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k)\}$ for the against-independence target over general time-varying cyclic graphs.
...
**Nature of the bound.** This is an **upper bound (converse)** on what any scheme can achieve; it is a *strong converse* in the against-independence case (the bound is the exact limiting exponent there). **Theorem D1★★ (§1.4) proves a matching achievability** for general time-varying fusion-free cyclic graphs, so the against-independence bound is **attained** — the loop is closed.
...
**(B) General pair (converse).** Apply the **Shimokawa–Han–Amari** converse: for a rate-$\Gamma$ one-sided description, $E_k\le\theta_{\mathrm{SHA}}(\Gamma)$ (§1.0-def-SHA). As corrected in §1.0-def-SHA, $\theta_{\mathrm{SHA}}$ and $\theta_{\mathrm{IB}}$ solve different problems and are **not universally ordered**; for the general pair the correct converse functional is $\theta_{\mathrm{SHA}}(\Gamma)$, which **equals** $\theta_{\mathrm{IB}}(\Gamma)$ only in the against-independence case (the D1★★ target).
...
## 1.4 — Theorem D1★★: Flawless Achievability (general time-varying cyclic graphs; testing against independence)

**Target.** An explicit decentralized, fusion-free scheme over a **general time-varying directed graph with cycles** achieving $E_k(\theta)=\min\{E^{\mathrm{cen}}(\theta),\theta_{\mathrm{IB}}(\Gamma_k)\}$ at every node $k$, testing against independence. The construction is **Type-Preserving Network Coding (TPNC)** — three layers: (i) distributed quantize-and-bin compression at the IB point; (ii) random linear network coding over $GF(q)$ transporting bins across the cyclic time-varying graph at the min-cut rate; (iii) joint-typicality testing at $k$.

### 1.4.0 Standing assumptions
[A1] **Against independence:** $H_0:P_{X_{[N]}Y}$ with $\{X_i\}$ conditionally independent given $Y$; $H_1:\big(\prod_iP_{X_i}\big)P_Y$. [A2] $Y$ is observed at the deciding node $k$ (each node decides for itself using $Y$ and routed descriptions — still fusion-free, no node holds others' raw data). [A3] Finite alphabets (Polish spaces handled by standard fine quantization; the Gaussian/lattice case is §1.6). [A4] $\{G_t\}$ stationary ergodic, **directed, cycles allowed**.

> **Theorem D1★★ (closed achievability).** Under [A1]–[A4], for every node $k$ and every $\delta>0$ there is a finite-block TPNC scheme with Type-I $\le\varepsilon$ and Type-II exponent $\ge\min\{E^{\mathrm{cen}}(\theta),\theta_{\mathrm{IB}}(\Gamma_k)\}-\delta$. Hence $E_k(\theta)=\min\{E^{\mathrm{cen}}(\theta),\theta_{\mathrm{IB}}(\Gamma_k)\}$, **matching the converse D1★ with zero gap.**

The proof is Lemmas A–C-D1, composed in §1.4.4.

### Lemma A-D1 (Type-Preserving Network Encoding over $GF(q)$)
**Construction.** Fix block length $n$. Each agent $i$ uses the IB-optimal test channel $p^\star(u_i\mid x_i)$ attaining the boundary of $\theta_{\mathrm{IB}}$ at its allocated rate $R_i$ (allocation by the water-filling of §1.6; on the binding cut $S$, $\sum_{i\in\mathrm{src}(S)}R_i=\Gamma_k$). Agent $i$: (1) draws a codebook $\mathcal C_i=\{u_i^n(1),\dots,u_i^n(2^{nI(U_i;X_i)})\}$ i.i.d. $\sim\prod p^\star(u_i)$; (2) quantizes $X_i^n\mapsto$ index $w_i$ (the $u_i^n$ jointly typical with $x_i^n$); (3) **bins** $b_i=\phi_i(w_i)\in GF(q)^{nR_i/\log q}$ by a uniform random hash (Wyner–Ziv/Berger–Tung binning), reducing the rate to $R_i$.
**Edge map (per directed edge, per round).** On $(i,j)$ at round $t$ the transmitted symbol is a uniform random $GF(q)$-linear combination of everything $i$ holds,
$$y_{ij,t}=\sum_{e\in\mathrm{In}(i,t)}\kappa^{(e)}_{ij,t}\,y_e\ +\ \sum_{\ell}\mu^{(\ell)}_{ij,t}\,b_{i,\ell}\pmod q,\qquad \kappa,\mu\sim\mathrm{Unif}\,GF(q),$$
random linear network coding (Ho–Médard–Koetter–Karger–Effros–Shi–Leong 2006; Koetter–Médard 2003). **Cycles** are handled by the time-expanded graph (infinite-capacity memory edges $(i,t)\!\to\!(i,t{+}1)$), so the global transfer matrix $M$ from source bins to $k$ is well-defined on the DAG.
**Claim (type preservation).** Algebraic mixing does not destroy the joint typicality required for the test: at $k$ the bins $\{b_i\}$ are recovered **exactly** (Lemma B-D1), the recovered indices index the true jointly-typical $\{u_i^n\}$, and the empirical joint type of $(\{u_i^n\},Y^n)$ converges to $p^\star_{U_{[N]}Y}$.
*Proof.* RLNC delivers $\{b_i\}$ losslessly iff $M$ has full column rank, which holds w.p. $\ge 1-|E|/q$ (Schwartz–Zippel over the random coefficients). Recovery is **exact** (zero distortion on the finite-field bins), so recovered indices equal transmitted indices and the joint type of the indexed codewords is invariant under transport. By the Markov lemma (Berger–Tung), $(\{u_i^n\},Y^n)$ are jointly $p^\star$-typical w.h.p. $\;\square$

### Lemma B-D1 (Distributed Independent-Codebook Joint Decoding)
**Problem.** Agents build $\mathcal C_i$ **independently** (only public hash seeds shared); show $k$ jointly decodes with no rate loss.
**Construction.** Over the block, $k$ collects its incoming linear observations $\mathbf y_k=M\,\mathbf b\ (\bmod\,q)$, $\mathbf b=(b_1,b_2,\dots)$. The number of independent $GF(q)$-equations reaching $k$ equals the time-expanded min-cut $=n\Gamma_k/\log q$ (Lemma C-D1). Since $M$ is full column-rank w.h.p., the system is solvable for $\mathbf b$ **iff** $\sum_iR_i\le\Gamma_k$; solve $\hat{\mathbf b}=M^{-1}\mathbf y_k$ by Gaussian elimination over $GF(q)$.
**Un-binning / joint-typicality.** Given $\hat{\mathbf b}$, $k$ recovers $\hat w_i$ as the unique $u_i^n$ in bin $\hat b_i$ jointly typical with $Y^n$ (Wyner–Ziv success w.h.p. because the binning rate satisfies $R_i\ge I(U_i;X_i)-I(U_i;Y)$). **Information-spectrum guarantee (no alignment loss):** independently generated codebooks admit a joint typical-set decoder with vanishing error provided the **sum** rate across the cut meets the min-cut (Han–Verdú information-spectrum method; Han 2003) — the per-codebook randomness is averaged by the spectral inf-information rate, which for stationary sources equals $I(U_{[N]};Y)$. Hence $k$ recovers $\{w_i\}$ and the joint type, realizing **exactly $\Gamma_k$** nats/sample of relevant information. $\;\square$

### Lemma C-D1 (Ergodic Time-Varying Cut Aggregation)
**Problem.** $G_t$ changes per round; show block coding over $T$ rounds aggregates the cuts to $\Gamma_k$ with **no** vanishing gap.
**Construction.** Run RLNC over super-blocks of $T$ rounds on the time-expanded DAG $\mathcal G_{1:T}$. By min-cut–max-flow for the static DAG (ACLY 2000), the source$\to k$ max-flow equals $\sum_{t=1}^T\min_{S\in\mathrm{Cut}(k)}\sum_{(i,j)\in S}C_{ij}(t)$. Dividing by $T$ and invoking [A4] ergodicity, $\tfrac1T(\text{max-flow})\to\Gamma_k$ a.s. (Birkhoff). RLNC achieves any rate $<$ max-flow with error $\le|E|T/q$; choosing $q=q(T)\to\infty$ (e.g. $q=T^2$) sends transport error to $0$ **faster than any fixed exponent**, at a field-size rate cost $\tfrac{\log\log q}{\log q}\to0$. Thus the per-sample relevant rate to $k$ converges to $\Gamma_k$ with transport error exponent $\to\infty$. $\;\square$

### 1.4.4 Composition: proof of D1★★
By Lemma C-D1, for any $\eta>0$ choose $T,q$ giving delivered rate $\ge\Gamma_k-\eta$ and transport error $\le e^{-n\zeta}$ with $\zeta$ as large as desired. By Lemmas A–B-D1, $k$ recovers the rate-$(\Gamma_k-\eta)$ IB-optimal descriptions and $Y^n$, then runs the joint-typicality / log-likelihood test $T_n=\mathbf 1\{(\{u_i^n\},Y^n)\in\mathcal T^n_{p^\star}\}$. By Ahlswede–Csiszár achievability (binding-link case) and **Rahman–Wagner (2012)** tightness of binning for distributed testing against independence, this test's Type-II exponent at Type-I $\le\varepsilon$ equals $\theta_{\mathrm{IB}}(\Gamma_k-\eta)$. Transport adds $\le e^{-n\zeta}$ to Type-II; choosing $\zeta>\theta_{\mathrm{IB}}$ leaves the exponent $=\theta_{\mathrm{IB}}(\Gamma_k-\eta)$. The full-data ceiling gives $\le E^{\mathrm{cen}}$. Hence the exponent is $\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k-\eta)\}$. Let $\eta\to0$: by continuity of the concave $\theta_{\mathrm{IB}}$, the achievable exponent $\to\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k)\}$. Since $E_k$ is the supremum over schemes, $E_k(\theta)=\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k)\}$. $\;\blacksquare$

**Zero-gap statement.** Converse (D1★) $=$ achievability (D1★★) $=\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k)\}$ for general time-varying directed **cyclic** graphs, testing against independence. The "$\eta\to0$" is the standard asymptotic-achievability limit (the exponent is a supremum over block schemes); there is **no $\varepsilon$-overhead in the theorem statement**.

**Scope (precise).** D1★★ closes the **against-independence** target — exactly the converse's domain [A1]. The **general-pair** distributed-HT exponent is a structurally different problem (even its single-link version is unresolved in the literature); it is governed by the $\theta_{\mathrm{SHA}}$ converse and is **not** within this target. This is a scoping statement, not an open gap in D1★★.
...
**Distributed dispersion — a separate second-order refinement (not part of the exponent-closure target).** The **first-order exponent** is fully closed (D1★ = D1★★). The finer **second-order** (finite-blocklength) behavior is a distinct, deeper question. *Conjectured form:* $-\ln\beta_n^{(k)}=n\,\theta_{\mathrm{IB}}(\Gamma_k)-\sqrt{nV_{\mathrm{dist}}(\Gamma_k)}\,\Phi^{-1}(\varepsilon)+o(\sqrt n)$ with a **distributed information-variance** $V_{\mathrm{dist}}$ combining (i) the centralized $V$ along the binding cut and (ii) a quantization-dispersion term from the rate-$\Gamma_k$ description. This second-order term is **not** required for the closed-loop first-order theorem; it is flagged only because finite-$n$ experiments (§1.7) use the centralized $V$ as the bias-correction baseline. **Closest techniques** for a future second-order analysis: Tomamichel–Tan (2013) one-shot HT + Watanabe second-order distributed source coding.
...
**What would be observed if D1★ is tight:** $\widehat E_k$ values roughly equal across topologies at equal $\Gamma_k$. This is the **achievability proved in Theorem D1★★**, here checked empirically (not a prediction of the converse D1★ alone).
...
**What this experiment tests:** (i) the converse is not violated by any topology (primary purpose); (ii) the tightness proved by **Theorem D1★★** — that $\Gamma_k$ is a sufficient statistic for the achievable exponent — is empirically confirmed (secondary).
...
- **Primary: IEEE Trans. Information Theory.** Justification: D1★/D1★★ form a converse–achievability pair in the lineage of Ahlswede–Csiszár (1986) and Rahman–Wagner (2012). **AE subfield:** multiterminal information theory / hypothesis testing. **AE will demand:** matching achievability — **supplied** by Theorem D1★★ (§1.4). **Plan:** present the closed converse–achievability pair plus the §1.7 numerical confirmation.
...
## 1.11 — Closed-Loop Status Table (Direction 1)

| Regime | Converse | Achievability | Status |
|---|---|---|---|
| $N=1$, static, $\Gamma\to\infty$ | Stein | Stein | CLOSED (classical) |
| Static star + fusion center, vs. independence | AC '86 | AC '86 / Aguerri–Zaidi '19 | CLOSED (classical) |
| **Time-varying, general directed cyclic graph, vs. independence** | **D1★** ($\le\theta_{\mathrm{IB}}(\Gamma_k)$) | **D1★★** (TPNC, §1.4) | **CLOSED (Theorem D1★★)** |
| Multiterminal distributed cut, vs. independence | D1★ | D1★★ via Rahman–Wagner binning | CLOSED (Theorem D1★★) |
| Time-varying, **general pair** (not vs. independence) | D1★ via $\theta_{\mathrm{SHA}}$ | — | Converse only (structurally different problem; even its single-link exponent is unresolved in the literature) |
| Second-order dispersion (finite-$n$) | — | — | First-order CLOSED; dispersion is a separate finer question (§1.5.1) |

**Headline:** the row "**Time-varying, general graph, vs. independence**" is **CLOSED (Theorem D1★★)** — converse $=$ achievability $=\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k)\}$, zero gap.

## 1.12 — Empirical Blueprint (ready for A100 hardware)

**D1-GPU-E1 — Verify the $\theta_{\mathrm{IB}}$ exponent with a decentralized GNN over a cyclic topology.**
- **Setup:** $N=8$ agents on a **directed cyclic** graph (bidirectional ring with chords). Each agent runs a small GNN encoder $U_i=\mathrm{GNN}_\phi(X_i)$ emitting a $\lceil\Gamma_k\rceil$-nat message; messages are combined by a differentiable $GF(q)$-style linear network-coding aggregation layer and routed per $G_t$.
- **Data:** the Gaussian against-independence model of §1.6-AI ($\rho_i$ set by the asymmetric water-filling of §1.6), with $Y$ at each decoding node.
- **Training:** end-to-end minimize Type-II error at Type-I $\le\varepsilon$, with a **hard $\Gamma_k$ bottleneck** (vector-quantization / Gumbel-softmax codebook of size $e^{\Gamma_k}$).
- **Measurement:** estimated exponent $\widehat E_k$ vs $\Gamma_k$; overlay the closed form $\theta_{\mathrm{IB}}(\Gamma_k)$.
- **Pass criterion (verifies D1★★):** $\widehat E_k\to\theta_{\mathrm{IB}}(\Gamma_k)$ within CI; the GNN should **rediscover** the TPNC structure (linear aggregation of per-agent LLR statistics), empirically confirming Lemma A-D1.
- **Hardware:** A100 for the GNN; `torch.distributed` over the 48 CPU threads enforces per-edge rate.
- **Falsification:** $\widehat E_k>\theta_{\mathrm{IB}}(\Gamma_k)+\delta_{\mathrm{CI}}$ at any node falsifies the converse D1★ (hence the closed loop).

## CLOSED-LOOP STORY: READY FOR EXPERIMENTAL VALIDATION (Direction 1)

**The narrative.** Direction 1 is mathematically closed for its target. The converse **D1★** proves no decentralized rate-constrained scheme over *any* time-varying directed cyclic graph beats $\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k)\}$; the achievability **D1★★** exhibits an explicit Type-Preserving Network Coding scheme that attains it. **Upper limit $=$ lower limit $=\min\{E^{\mathrm{cen}},\theta_{\mathrm{IB}}(\Gamma_k)\}$** for testing against independence with conditionally-independent observations — the broadest setting in which the problem is information-theoretically determined.

**Pre-empting the hostile TIT reviewer.** *Attack: “Cyclic graphs cause routing ambiguity; the network-coding step hides a gap.”* **Defense:** Lemma A-D1 routes by random linear network coding over $GF(q)$ on the **time-expanded DAG** — cycles become acyclic memory edges; the transfer matrix $M$ is full column-rank w.p. $\ge1-|E|/q$ (Schwartz–Zippel); finite-field bin recovery is **exact** (zero distortion), so the joint type, hence the test exponent, is provably invariant under transport (Markov lemma). *Attack: “Independent codebooks can’t be jointly decoded without rate loss.”* **Defense:** Lemma B-D1 uses the Han–Verdú information-spectrum decoder, whose spectral inf-information rate equals $I(U_{[N]};Y)$ for stationary sources (zero alignment loss), and Rahman–Wagner (2012) prove binning is **optimal** for distributed testing against independence — the achieved exponent is exactly $\theta_{\mathrm{IB}}(\Gamma_k)$. *Attack: “Time variation breaks the min-cut.”* **Defense:** Lemma C-D1 aggregates per-round cuts by Birkhoff’s ergodic theorem on the time-expanded DAG, with field-size schedule $q=T^2$ driving transport error below any fixed exponent at $o(1)$ rate cost. *Honest scope note (not a gap):* the **general-pair** test is a different problem ($\theta_{\mathrm{SHA}}$ converse); it is outside the against-independence target and is not claimed.

**The next phase.** The theory is final. The sole remaining work is the hardware-accelerated empirical confirmation of §1.12 (the GNN rediscovering TPNC and tracing $\theta_{\mathrm{IB}}(\Gamma_k)$). No further theoretical development is required for the against-independence target.
