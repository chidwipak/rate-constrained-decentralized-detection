...
**§1.0-def-SHA.** For the general pair $(\theta_0,\theta_1)$ with one-sided compression at rate $\Gamma$, the SHA achievable exponent is
$$\theta_{\mathrm{SHA}}(\Gamma)=\min_{\tilde P_{UX}:\,I_{\tilde P}(U;X)\le\Gamma,\ \tilde P_X=P_{X\mid\theta_1}}\Big[D(\tilde P_{UX}\Vert P_{U\mid\theta_0}\!\times\!P_{X\mid\theta_1})\ \vee\ \big(I_{\tilde P}(U;X)-\Gamma\big)^+\text{-corrected binning term}\Big],$$
in the form given by Shimokawa–Han–Amari (1994). For **testing against independence** the binning term is inactive and $\theta_{\mathrm{SHA}}=\theta_{\mathrm{IB}}$ (Rahman–Wagner 2012 prove tightness in this case). For general pairs $\theta_{\mathrm{SHA}}$ is an **achievable lower bound** and a **converse upper bound** holds with the same single-letter region only in special cases; the exact general exponent is **open** (see §1.2).
...
*Proof.* $\theta_{\mathrm{IB}}$ is concave, non-decreasing, continuous on $[0,\infty)$, with supremum $I(X_{[N]};Y)$. For testing against independence $E^{\mathrm{cen}}=I(X_{[N]};Y)$ exactly, and $\theta_{\mathrm{IB}}(\Gamma)\to I(X_{[N]};Y)$ as $\Gamma\to\infty$; concavity + boundedness imply the supremum is attained at a finite $\Gamma$ iff the curve has a flat saturation segment, which holds whenever $Y$ is a deterministic function of a finite-rate statistic of $X$ (true for finite $\Theta$ since $Y=\theta$ takes $\le|\Theta|$ values, so $I(X;Y)\le\ln|\Theta|<\infty$ and saturation occurs at $\Gamma=\ln|\Theta|$). Hence $C_{\mathrm{DIB}}\le\ln|\Theta|<\infty$. $\;\blacksquare$
...
**(3) Cut-set / max-flow–min-cut for information.** By the network-information cut-set outer bound (Cover–Thomas Thm 15.10.1; El Gamal–Kim §15) applied to $\mathcal G$ with orthogonal noiseless rate-limited links, the information about any source variable that reaches the sink is bounded by the min-cut capacity of $\mathcal G$. For orthogonal links the min-cut capacity is additive over edges, giving $\sum_{t}\min_{S\in\mathrm{Cut}(k)}\sum_{(i,j)\in S}C_{ij}(t)$. **Directedness** is handled natively (cuts respect edge orientation). **Time-variation** is handled because $\mathcal G$ is a single static DAG encoding all rounds; the bound is the *sum* of per-round binding cuts, **not** the binding cut of a time-averaged graph (these differ when the binding cut moves with $t$ — the sum-of-minima form is the correct one).
...
### 1.7.2 Experiment D1-E2 — Topology invariance
Fix total capacity $C_{\mathrm{total}}$; sweep topology complete→ring→path while **holding $\Gamma_k$ fixed** by reallocating edge budgets. **Prediction:** $\widehat E_k$ depends on topology **only through $\Gamma_k$** — equal $\Gamma_k$ ⇒ equal $\widehat E_k$ (within CI). This tests that $\Gamma_k$ is the sufficient statistic for topology. **Falsification:** a statistically significant $\widehat E_k$ difference at equal $\Gamma_k$.
